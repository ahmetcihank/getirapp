package org.getir.domain.customer;

import com.google.common.collect.Sets;
import lombok.Getter;
import org.springframework.security.core.authority.SimpleGrantedAuthority;

import java.util.Set;
import java.util.stream.Collectors;

import static org.getir.domain.customer.ApplicationPermission.READ;
import static org.getir.domain.customer.ApplicationPermission.WRITE;


public enum RoleType {

    CUSTOMER(Sets.newHashSet(READ)),
    SUPERUSER(Sets.newHashSet(READ, WRITE));

    @Getter
    private Set<ApplicationPermission> authorityTypes;

    RoleType(Set<ApplicationPermission> authorityTypes) {
        this.authorityTypes = authorityTypes;
    }

    Set<SimpleGrantedAuthority> getAuthorities() {
        Set<SimpleGrantedAuthority> permissions = this.getAuthorityTypes().stream()
                .map(aut-> new SimpleGrantedAuthority(aut.getPermission()))
                .collect(Collectors.toSet());

        permissions.add(new SimpleGrantedAuthority("ROLE_" + this.name()));

        return permissions;
    }

}
